  ...
  apply (compiler) {
    ...
    var getJsFiles = function (assets, filepath) {
      ...
      if (options.devServer) {
        files['webpack-dev-server'] = url.URL(prefix, 'webpack-dev-server.js')
      }
...
const withPrefix = (prefix) => (ext) => (assets, filepath) =>
  Object.keys(assets)
    .reduce((acc, name) => {
      ...
      const chunk = ((_.isArray(assets[name])) ? assets[name] : [assets[name]])
        ...
        .reduce(function (_, filename) {
          return (!prefix)
            ...
            : url.URL(prefix, path.join(filepath, filename))
        }, undefined)
